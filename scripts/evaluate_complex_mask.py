...
from ha_denoise.features import enhance_with_complex_mask, extract_features, feature_config_from_dict, match_loudness, rms_ratio
from ha_denoise.metrics import si_sdr
from ha_denoise.model import TinyComplexMaskTCN
...
def load_model(checkpoint: str, device: str):
    ...
    model = TinyComplexMaskTCN(
        ...
        int(cfg_d.get("freq_bins", cfg.n_fft // 2 + 1)),
        int(cfg_d["channels"]),
        int(cfg_d["blocks"]),
        int(cfg_d["kernel_size"]),
        float(cfg_d.get("mask_scale", 2.0)),
    )
...
def process_one(
    ...
    complex_mask = model(feat).squeeze(0)
    enhanced = enhance_with_complex_mask(reference, complex_mask, cfg)
    n = min(mix.shape[-1], clean.numel(), reference.numel(), enhanced.numel())
    noisy = mix[0, :n]
    ...
    return sr, noisy, reference, clean, enhanced, spatial_info, float(loudness_gain.detach().cpu())
...
def main() -> None:
    ...
    with torch.no_grad():
        for mix_path in tqdm(mix_files):
            sr, noisy, reference, clean, enhanced, spatial_info, gain = process_one(
                model,
                cfg,
                mix_path,
                args.device,
            ...
            if save_dir:
                write_wav(save_dir / mix_path.name.replace("mix_", "complex_"), sr, enhanced.detach().cpu().numpy())
            ...
            listen_rows.append(
                {
                    ...
                    "spatial_frontend": spatial_info.get("mode"),
                    "beamform_lag_samples": spatial_info.get("lag"),
                    "reference_si_sdr": row.get("reference_si_sdr"),
                    "loudness_gain": gain,
