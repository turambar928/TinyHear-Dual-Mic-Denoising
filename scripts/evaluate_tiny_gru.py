...
from ha_denoise.features import enhance_with_mask, extract_features, feature_config_from_dict, match_loudness, rms_ratio
from ha_denoise.metrics import si_sdr
from ha_denoise.model import TinyGRUDenoiser
from ha_denoise.spatial import apply_spatial_frontend


def load_model(checkpoint: str, device: str) -> tuple[TinyGRUDenoiser, object]:
    ...
    model = TinyGRUDenoiser(
        ...
        int(cfg_d["bands"]),
        int(cfg_d["hidden"]),
        int(cfg_d.get("layers", 1)),
        float(cfg_d.get("min_gain", 0.04)),
        float(cfg_d.get("max_gain", 1.0)),
    )
...
def process_one(model, cfg, mix_path: Path, device: str, loudness_match: bool, target_rms_ratio: float, max_gain_db: float):
    ...
    reference, spatial_info = apply_spatial_frontend(mix, cfg, max_lag=8, analysis_samples=cfg.sample_rate // 2)
    feat = extract_features(mix, cfg).transpose(0, 1).unsqueeze(0)
    gain, vad = model(feat)
    mask = gain.squeeze(0).transpose(0, 1)
    enhanced = enhance_with_mask(reference, mask, cfg)
    n = min(mix.shape[-1], reference.numel(), clean.numel(), enhanced.numel())
    noisy = mix[0, :n]
    ...
    if loudness_match:
        enhanced, loudness_gain = match_loudness(reference, enhanced, target_rms_ratio, max_gain_db)
    return sr, noisy, reference, clean, enhanced, spatial_info, float(vad.mean().detach().cpu()), float(loudness_gain.detach().cpu())
...
def main() -> None:
    ...
    with torch.no_grad():
        for mix_path in tqdm(mix_files):
            sr, noisy, reference, clean, enhanced, spatial_info, vad_mean, gain = process_one(
                model,
                cfg,
                mix_path,
                args.device,
            ...
            rows.append(
                {
                    ...
                    "reference_si_sdr": float(si_sdr(reference.detach().cpu(), clean.detach().cpu())),
                    "noisy_si_sdr": noisy_score,
                    "enhanced_si_sdr": enhanced_score,
                    "si_sdr_improvement": enhanced_score - noisy_score,
                    "output_input_rms_ratio": float(rms_ratio(reference.detach().cpu(), enhanced.detach().cpu())),
                    "vad_mean": vad_mean,
                    "loudness_gain": gain,
                }
            )
            if save_dir:
                write_wav(save_dir / mix_path.name.replace("mix_", "tiny_gru_"), sr, enhanced.detach().cpu().numpy())
    ...
    summary = {
        ...
        "mean_vad": sum(row["vad_mean"] for row in rows) / len(rows),
    }
